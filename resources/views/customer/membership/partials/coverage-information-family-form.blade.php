...
<form method="POST" action="{{ route('customer.membership.coverage.update') }}" enctype="multipart/form-data" class="space-y-6" x-data="familyCoverageForm(@js($dependentRows))">
    ...
    <div class="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-md shadow-slate-200/50 ring-1 ring-slate-100">
        <details class="group border-b border-sky-100 bg-sky-50/80">
            ...
            <p class="border-t border-sky-100 px-6 pb-3 pt-2 text-xs leading-relaxed text-sky-900">
                {{ \App\Support\CoverageFormTranslations::t('family_intro_short') }}
                {{ \App\Support\CoverageFormTranslations::t('family_intro_extended') }}
            </p>
        </details>
    ...
    <div class="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-md shadow-slate-200/50 ring-1 ring-slate-100">
        <div class="hero-panel-header border-b border-slate-100 px-6 py-4">
            <h2 class="text-sm font-semibold text-slate-900">{{ \App\Support\CoverageFormTranslations::t('photo_id') }} &amp; {{ \App\Support\CoverageFormTranslations::t('passport') }}</h2>
            <p class="mt-1 text-xs text-slate-500">{{ \App\Support\CoverageFormTranslations::t('file_upload_help') }}</p>
        </div>
        <div class="grid grid-cols-1 gap-6 p-6 sm:grid-cols-2">
            <div>
                ...
                @if ($profile?->photo_id_path)
                    <p class="mt-2 text-xs text-green-700">{{ \App\Support\CoverageFormTranslations::t('file_on_file') }}</p>
                @endif
            </div>
            <div>
    ...
    <div class="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-md shadow-slate-200/50 ring-1 ring-slate-100">
        <div class="hero-panel-header border-b border-slate-100 px-6 py-4">
            <h2 class="text-sm font-semibold text-slate-900">{{ \App\Support\CoverageFormTranslations::t('emergency_contact') }}</h2>
        </div>
    ...
    <div class="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-md shadow-slate-200/50 ring-1 ring-slate-100">
        <div class="hero-panel-header border-b border-slate-100 px-6 py-4">
            <h2 class="text-sm font-semibold text-slate-900">{{ \App\Support\CoverageFormTranslations::t('dependents') }}</h2>
            <p class="mt-1 text-xs text-slate-500">{{ \App\Support\CoverageFormTranslations::t('dependents_help') }}</p>
        </div>
        <div class="space-y-4 p-6">
                ...
                <div class="rounded-xl border border-slate-200 p-4">
                    <div class="mb-3 flex items-center justify-between gap-3">
                        <div class="text-sm font-semibold text-slate-900">
                            {{ \App\Support\CoverageFormTranslations::t('dependent_n') }}
                        ...
                        <button type="button" class="text-xs font-semibold text-red-700 hover:underline" x-show="dependents.length > 1" @click="removeDependent(index)">
                            {{ \App\Support\CoverageFormTranslations::t('remove') }}
                        </button>
                    </div>
                        ...
                        <div>
                            ...
                            <select class="{{ $inputClass }}" x-model="row.gender" :name="`dependents[${index}][gender]`" required>
                                ...
                                @foreach ($genderOptionKeys as $value => $labelKey)
                                    <option value="{{ $value }}">{{ \App\Support\CoverageFormTranslations::t($labelKey) }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
            ...
            <button type="button" @click="addDependent()" class="text-sm font-semibold text-hero-primary hover:underline">
                {{ \App\Support\CoverageFormTranslations::t('add_more') }}
            </button>
        </div>
    </div>
    ...
    <div class="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-md shadow-slate-200/50 ring-1 ring-slate-100">
        <div class="hero-panel-header border-b border-slate-100 px-6 py-4">
            <h2 class="text-sm font-semibold text-slate-900">{{ \App\Support\CoverageFormTranslations::t('notes') }}</h2>
        </div>
        <div class="p-6">
            <textarea name="notes" rows="4" class="{{ $inputClass }}" placeholder="{{ \App\Support\CoverageFormTranslations::t('notes_placeholder') }}">{{ old('notes', $profile?->notes) }}</textarea>
        </div>
    </div>

    <div class="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-md shadow-slate-200/50 ring-1 ring-slate-100">
        <div class="hero-panel-header border-b border-slate-100 px-6 py-4">
            <h2 class="text-sm font-semibold text-slate-900">{{ \App\Support\CoverageFormTranslations::t('insurance_section') }}</h2>
            <p class="mt-1 text-xs text-slate-500">{{ \App\Support\CoverageFormTranslations::t('insurance_section_help') }}</p>
        </div>
        <div class="space-y-4 p-6">
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
    ...
    <div class="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-md shadow-slate-200/50 ring-1 ring-slate-100">
        <div class="hero-panel-header border-b border-slate-100 px-6 py-4">
            <h2 class="text-sm font-semibold text-slate-900">{{ \App\Support\CoverageFormTranslations::t('medical_section') }}</h2>
        </div>
        <div class="space-y-4 p-6">
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
            ...
            <div>
                <p class="mb-2 text-sm font-semibold text-slate-900">{{ \App\Support\CoverageFormTranslations::t('medical_checklist') }}</p>
    ...
    <div class="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-md shadow-slate-200/50 ring-1 ring-slate-100">
        <div class="hero-panel-header border-b border-slate-100 px-6 py-4">
            <h2 class="text-sm font-semibold text-slate-900">{{ \App\Support\CoverageFormTranslations::t('terms_section') }}</h2>
        </div>
        <div class="space-y-4 p-6">
