...
class HeroPortalWebhookService
{
    ...
    private function buildPayload(Plan $plan, array $requestData, array $parsed, string $subscriptionId, ?Carbon $coverageStart = null): array
    {
        ...
        $recordType = trim((string) ($requestData['record_type'] ?? ''));
        if ($recordType === '') {
            $recordType = HeroPortalPlanMapper::recordTypeForGatewayPlanId($plan->plan_id);
        }

        $companyName = trim((string) ($requestData['company'] ?? $requestData['company_name'] ?? ''));
        ...
        return [
            ...
            'company_billing_email' => (string) ($requestData['email'] ?? ''),
            'company_phone' => (string) ($requestData['phone'] ?? ''),
            'company_city' => (string) ($requestData['city'] ?? ''),
            'company_country' => (string) ($requestData['country'] ?? ''),
            ...
            'customer' => [
                ...
                'first_name' => (string) ($requestData['first_name'] ?? ''),
                'last_name' => (string) ($requestData['last_name'] ?? ''),
